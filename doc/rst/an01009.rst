...
- The `XK-AUDIO-316-MC` board. This board has a jumper (J24)
  over which the 5V supply to the core power supply, upstream of the DC-DC converter, can be monitored. Note
...
This command will cause two binaries (.xe files) to be generated in the `app_an01009/bin` directory
...
* **Reducing frequencies** Execute all the software on one physical tile.
  The `xcore` switch and other tile can both be slowed down. In this case, the frequency of core 0
  is reduced to 9 MHz which means it can still execute low speed tasks.
...
* **XUD thread mode**  Disable `FAST` mode in ``XUD_Manager_loop`` in (``lib_xud``) and instead run the XUD thread in
  `HIGH PRIORITY` mode which ensures it always receives 1/5 of the core MIPS. For further information about the operation
...
Choosing a slow, but still functional, core speed of 9 MHz allows the I²C master task on that tile
...
However it also increases power consumption because the thread is always scheduled, event when waiting on an event. In low-power
...
By switching to USB Audio Class 1, ``lib_xua`` automatically uses full-speed USB when calling ``lib_xua``. This can be affected by
...
Standard + clock reduction               150 mW       22 mW     172 mW
...
.. note::
    ...
    is only a milliwatt or two. Likewise, when not streaming, power is similar because USB must stay alive.
...
* Minimise power supply on USB suspend [#f2]_.

.. [#f1] Note this does *not* affect the secondary PLL output pin muxed with IO pin `X1D11` of
   tile[1] which continues to operate.

.. [#f2] Suspend, by default, does not offer significantly reduced power consumption because
   the `xcore` stays powered up waiting to receive the resume signal. Work is underway at XMOS to
   investigate reducing suspend power however ``lib_xua`` already provides the ``UserHostActive()``
   callback (derived from the ``lib_xud`` suspend/resume callbacks) which can be used to shut
   down external hardware to reduce suspend power consumption. 
...
power consumption by approximately 62% compared with the baseline reference design whilst still
providing high-quality, bit-perfect USB audio streaming.
