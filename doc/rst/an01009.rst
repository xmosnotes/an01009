...
* **XUD thread mode**  Disable `FAST` mode in ``XUD_Manager_loop`` in (``lib_xud``) and instead run the XUD thread in
  `HIGH PRIORITY` mode which ensures it always receives 1/5 of the core MIPS.

* **Using full-speed USB** Reduction of power by lowering the audio sample frequency to 48 kHz and use of full speed USB rather than high-speed. The latter is important as it affects the PHY power consumed.
...
XUD normally sets ``set_thread_fast_mode_on()`` which improves timing in the core USB device library.
However it also increases power consumption because the thread is always scheduled. In low-power
USB audio applications it can be acceptable to disable this. 
...
By switching to USB Audio Class 1, ``lib_xud`` automatically uses full-speed USB. This can be affected by
...
Standard + clock + buffer                105 mW       22 mW     127 mW
...
power consumption by approximately 62% compared with the baseline reference design.
