...
* **Using full-speed USB**
  This helps active and non-streaming power consumption. It provides a reduction of power through use of full speed USB rather than high-speed. This is important as it affects the PHY power consumed. ``lib_xua``     follows the standards and uses high-speed (480 Mbps) USB for Audio Class 2 and full-speed (12 Mbps) for Audio Class 1, although Audic Class 2 over full-speed is supported.

* **Extreme chip power down in USB Suspend** When the `xcore` receives a suspend notification from the host it is possible to reduce power significantly to a point where the `xcore` power draw is significantly below the USB Suspend specification of <= 2.5 mA from the bus by clocking the whole chip down to just a few MHz.
...
The provided source file ``src/extensions/power_down.xc`` contains a function which is run at audio hardware initialisation to reduce the active power consumption. 
...
by adjusting the following define in `CMakeLists.txt`::
    ...
    -DAN01009_CLOCK_DOWN_SWITCH_AND_UNSED_TILE=1
...
a macro to remove the call to ``set_thread_fast_mode_on()``.
 ...
 .. literalinclude:: ../../app_an01009/src/extensions/xud_conf.h
    ...
    :start-after: #if AN01009_DISABLE_XUD_FAST_MODE
    :end-before: #endif

In addition, to ensure that XUD always receives sufficient MIPS in the cases where more than 5 hardware threads
are used on the XUD tile, the following defines are set in the ``CMakeLists.txt``::

    -DAN01009_DISABLE_XUD_FAST_MODE=1
...
USB audio device by limiting it to 96 kHz which is the maximum for the given bus bandwidth. 
...
still needs to respond to endpoint 0 requests and Start of Frame (SoF) packets from the host.
...
- Ensure the audio hardware is disabled and powered down and I²S is halted and the MCLK PLL is disabled. Requires ``XUA_LOW_POWER_NON_STREAMING`` to be set.
...
FS Class 1, USB Suspend                                  8 mW        2 mW      10 mW
======================================================= =========== ========== ========
...
baseline USB audio class 2 reference design, whilst still providing high-quality, bit-perfect USB audio playback.

A further 10 to 13% of ``xcore.ai`` chip power may be saved when the device is enumerated, but not streaming, with the addition
of non-streaming power optimisations. Additionally the system power can be dramatically reduced by powering down the external audio hardware.

During USB suspend, the `xcore.ai`` chip power may be reduced to well below the nominal 15 milliwatts required by the USB specification.
