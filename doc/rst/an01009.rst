...
From an XTC command prompt run the following command::
...
* **Clock parts of the chip down** Execute all the software on one physical tile.
...
.. [#f2] Note this does *not* affect the secondary PLL output pin muxed with IO pin `X1D11` of
   tile[1]
