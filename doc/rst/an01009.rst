...
* **Event-based buffering** Ensure that as few instructions as possible are executed in the audio buffering subsystem
...
By switching to USB Audio Class ones, ``lib_xua`` automatically uses full-speed USB when calling ``lib_xua``. This can be affected by
...
power consumption by approximately 55% (or 62% if USB audio class 1 is used) compared with the 
