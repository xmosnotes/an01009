...
which is the main power draw of the chip, upstream of the DC-DC converter, can be monitored.
*Note that this excludes 5V supply to 3V3 and 1V8 pins.*
...
For each configuration, basic enumeration and playback was confirmed connected to the Windows host,
with power being measured during audio playback, idle (non-streaming) and during USB suspend which
was triggered by closing the lid of the laptop.
...
By default the `XMOS USB Audio Reference Design` sets the `bMaxPower` field to 500 mA (50) for
