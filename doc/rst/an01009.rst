...
* **Event-based buffering**
  ...
  by only checking shared memory variables when needed through use of a channel event. The define ``XUA_CHAN_BUFF_CTRL`` is
...
* **Audio shutdown when idle**
  ...
  be disabled (if used) which further saves power draw by the `xcore`.
...
a macro to remove the call to ``set_thread_fast_mode_on()``.
 ...
 .. literalinclude:: ../../app_an01009/src/extensions/xud_conf.h
    ...
    :start-after: #ifdef AN01009_CLOCK_DOWN_SWITCH_AND_UNSED_TILE
    :end-before: #endif
...
modifying ``xua_conf.h``.
 ...
 .. literalinclude:: ../../app_an01009/src/core/xua_conf.h
    ...
    :start-at: #ifdef AN01009_CLOCK_DOWN_SWITCH_AND_UNSED_TILE
    :end-at: #endif
...
.. note::
    The additional power required to stream at 192 kHz, 24 bit (only possible with Class 2 - 96 kHz, 24 bit limit for Class 1)
